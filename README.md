...
> An intelligent, ingredient-based recipe recommendation system designed to help users minimize food waste by finding recipes that maximize the use of their available pantry ingredients.
...
* **TF (Term Frequency):** Calculates how often an ingredient appears.
* **IDF (Inverse Document Frequency):** Penalizes ubiquitous ingredients (like salt and water) and boosts the weight of unique, defining ingredients (like saffron or tahini).
* Hyperparameters (`max_df=0.85`, `max_features=5000`) were tuned to eliminate noise and capture compound ingredients (e.g., "soy_sauce").
...
* The model utilizes a **Cosine distance metric** to expand a search radius and return the mathematically closest recipes.
...
The KNN model demonstrated a significant **88.9% improvement** over the baseline in top-3 precision, proving the effectiveness of the weighted vector space approach over simple ingredient overlap counting.
...
Dishcover/
├── data/
│   ├── recipes.csv               # Raw dataset (Add from Kaggle)
